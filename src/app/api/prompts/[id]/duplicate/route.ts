...
export async function POST(
    ...
    try {
        ...
        if (!originalPrompt.organizationId) {
            console.error(`Prompt ${id} does not have an associated organizationId.`);
        ...
        if (!sourceOrgAccess) {
            console.warn(`User ${userId} attempted to duplicate prompt ${id} from org ${originalPrompt.organizationId} without access.`);
